...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var light_index = max(in.block_light.x, in.block_light.y);
    let scaled_light = light_index *  0.066666666666666;
    var light_mapping = vec2<f32>(light_index, 0.0);
    var light = textureSample(t_lightmap, s_lightmap, light_mapping);
    if (sampleT.w < 0.1) {
        discard;
    }
